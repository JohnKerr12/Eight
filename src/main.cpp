...
#if defined(NDEBUG)
# error "Piece of Eight cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6f8f3c3d2ccd11b5e49f15727e8158858a011801a3fca19baa7ab17b827d5167");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Piece of Eight: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Piece of Eight Signed Message:\n";
...
{
    // Piece of Eight: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 88888); // Piece of Eight: 8.8k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Piece of Eight: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Piece of Eight: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Piece of Eight: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Piece of Eight: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1282e8526437bb85d72336a78fc164508c4007b7d715ab1ccf75be48a12631a0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Co-founder of Vine and HQ Trivia Colin Kroll is found dead";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1544989991;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085210178;

        if (fTestNet)
        {
            block.nTime    = 1544989977;
            block.nNonce   = 386046458;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5cc795bf7048245467459163ba7420351560b5acf1e6e4af82bf45251c674864"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc7, 0xba, 0xd5 }; // Piece of Eight: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Piece of Eight Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("POEMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static POEMiner(CWallet *pwallet)
{
    printf("Piece of Eight Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("piece of eight-miner");
    ...
    try { loop {
        ...
        printf("Running Piece of Eight Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Piece of Eight Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&POEMiner, pwallet));
}
